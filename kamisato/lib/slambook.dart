...
class _SlamBookFormState extends State<SlamBookForm> {
  ...
  void submitForm() {
    // Check if name is already in list of names, used lowercase for case-insensitive comparison
    bool nameExists = widget.friendsList.any((friend) => friend.containsKey(nameController.text));

    if (nameExists) {
      ScaffoldMessenger.of(context)
        ..removeCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text("Name already exists in the list")),
        );
      return;
    }
    ...
    if (!isValidNickname(nicknameController.text)) {
      ScaffoldMessenger.of(context)
        ..removeCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text("Invalid Nickname. It must start with a letter and it can only be one or two words.")),
        );
      return;
    }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      setState(() {
        ...
        // Add and append map to friendlist
        widget.friendsList.add({
          submittedName: [
            submittedName,
            submittedNickname,
            submittedAge,
            submittedIsSingle.toString(),
            submittedHappinessLevel.toString(),
            submittedDropdownValue,
            submittedMotto
          ]
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Kamisato Records"),
      ),
      body: Padding(
              ...
              children: <Widget>[
                ...
                Column(
                  children: [
                    ...
                    Center(
                        child: Image.asset("assets/ayato_writes.png",
                            height: 150, width: 150)),
                    const Center(
                      child: Text(
                        "Welcome to Kamisato Records!",
                        style: TextStyle(
                    ...
                    const Padding(
                      ...
                      child: Center(
                        child: Text(
                          "Enter your name, nickname, age, and civil status to be part of the Kamisato Records.",
